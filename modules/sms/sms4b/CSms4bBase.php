...
';
	...
	function CSms4bBase($login = '', $password = '')
	{
		...
		$this->gmt = "4";   // Ваш часовой пояс относительно всемирного координированного времени
		$this->serv_addr = "https://sms4b.ru";
		$this->UpdateSID();
		$senders = $this->GetSender();	// получаем символьные имена клиента
		$this->DefSender = $senders[0]; 	// отправитель по умолчанию
		return;
	}
...
**************************/
	...
	public function enCodeMessage($message)
	{
		...
		else
		{
			...
			$message=preg_split('//u',$message,-1,PREG_SPLIT_NO_EMPTY); // необходимо для выделения символов в кодировке UTF-8
	...
	{
		$addr = htmlspecialchars($addr);
		if(preg_match('/^([0-9]{1,10})$/',$addr)) //короткий
			return 9;
		elseif (preg_match('/^8([0-9]{10})$/',$addr)) // федеральный
			return 1;
		elseif (preg_match('/^([0-9]{11,15})$/',$addr)) // общий
			return 1;
		else// именной
			return 0;
	}
...
*/
	public function SendSMS($message, $to, $sender='', $IDOrder = 0, $Posting = 0, $TypeEvents = '')
	{
		...
		$to = $this->is_phone($to);
		$message = strval($message); // переводим сообщение в символьную форму (для случая, когда в сообщении одни цифры)
	...
	{
		// переводим сообщение в символьную форму (для случая, когда в сообщении одни цифры)
		$message = strval($message);

		//Получаем код текущей сессии для отправки sms
		$session = $this->GetSID();

		$sendingError = array();

		//получаем текущий часовой пояс и убираем поправку на зимнее/летнее время
		$gmt = $this->gmt;

		//Формат рассылки (-1 - неравномерная, -2 - равномерная)
		$code = -1;

		//формируем коды для вида номера (цифровой - 1, буквенный - 5)
		$ston = $this->get_ton($sender);
		$snpi = $this->get_npi($sender);

		//определяем каким способом кодировать сообщение
		$encoded = $this->get_type_of_encoding($message);

		//кодируем сообщение и определяем в каком стиле оно закодировано
		$body = $this->enCodeMessage($message);

		//Проверяем актуальность даты доставки
		$dateActual = $dateActual_p;

		//Стартап отправки сообщения
		$startUp = $startUp_p;

		//проверяем период отправки
		$period = $period_p;
		...
		//здесь храним результаты отсылки
		$results_of_package_send = array();

		//массив для хранения номеров на которые не удалось отправить сообщения
...
/*						$results_of_package_send['NOT_SEND'] += $currentChunkLength - $temp["result"];*/
	...
	{
		//если нету параметра в сессии вообще, то стартуем новую сессию
		...
		{
			//иначе проверяем, а не протухла ли сессия
